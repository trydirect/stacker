...
async fn given_step_command(world: &mut StepWorld, step: &cucumber::gherkin::Step, step_type: String) {
    ...
    world.stored_ids.insert("step_command_json".to_string(), serialized);
    world.stored_ids.insert("step_config".to_string(), serde_json::to_string(&config).unwrap());
}
...
async fn given_step_input(world: &mut StepWorld, step: &cucumber::gherkin::Step) {
    ...
    world.stored_ids.insert("step_input".to_string(), input_str.trim().to_string());
}
...
async fn given_success_result(world: &mut StepWorld, step: &cucumber::gherkin::Step) {
    ...
    world.stored_ids.insert("step_result_json".to_string(), serialized);
}
...
async fn when_command_roundtrip(world: &mut StepWorld) {
    ...
    let deserialized: StepCommand = serde_json::from_str(&json_str).expect("Deserialization failed");
    let reserialized = serde_json::to_string(&deserialized).unwrap();
    world.stored_ids.insert("step_command_roundtrip".to_string(), reserialized);
}
...
async fn when_result_roundtrip(world: &mut StepWorld) {
    ...
    let deserialized: StepResultMsg = serde_json::from_str(&json_str).expect("Deserialization failed");
    let reserialized = serde_json::to_string(&deserialized).unwrap();
    world.stored_ids.insert("step_result_roundtrip".to_string(), reserialized);
}
...
async fn then_result_status(world: &mut StepWorld, expected: String) {
    let json_str = world.stored_ids.get("step_result_roundtrip")
...
async fn then_result_has_output(world: &mut StepWorld) {
    ...
    let result: StepResultMsg = serde_json::from_str(json_str).unwrap();
    assert!(result.output_data.is_some(), "Expected output data to be present");
}
...
async fn when_execute_step(world: &mut StepWorld) {
    ...
    let input = world.stored_ids.get("step_input")
        .map(|s| serde_json::from_str(s).unwrap())
        .unwrap_or(cmd.input_data.clone());

    match step_executor::execute_step(&cmd.step_type, &cmd.config, &input).await {
        Ok(output) => {
            world.stored_ids.insert("exec_output".to_string(), serde_json::to_string(&output).unwrap());
            world.stored_ids.insert("exec_success".to_string(), "true".to_string());
        }
        Err(err) => {
            world.stored_ids.insert("exec_error".to_string(), err);
            world.stored_ids.insert("exec_success".to_string(), "false".to_string());
        }
    }
}
...
async fn then_exec_success(world: &mut StepWorld) {
    ...
    assert_eq!(success, "true", "Expected execution to succeed but got error: {:?}",
...
async fn then_exec_fail_with(world: &mut StepWorld, expected: String) {
    ...
    assert!(error.contains(&expected), "Error '{}' does not contain '{}'", error, expected);
}
...
async fn then_output_contains(world: &mut StepWorld, key: String, value: String) {
    let output_str = world.stored_ids.get("exec_output").expect("No execution output");
    ...
    assert!(matches, "output[{}] = {:?}, expected {:?}", key, actual, value);
}
...
async fn when_record_failures(world: &mut StepWorld, count: u32) {
    let cb = world.circuit_breaker.as_mut().expect("No circuit breaker initialized");
    for _ in 0..count {
        cb.record_failure();
    }
}
...
async fn then_cb_state(world: &mut StepWorld, expected: String) {
    ...
    assert!(state.contains(&expected), "CB state '{}' does not match '{}'", state, expected);
}

#[then("the circuit breaker should reject requests")]
async fn then_cb_rejects(world: &mut StepWorld) {
    let cb = world.circuit_breaker.as_mut().expect("No circuit breaker");
    assert!(!cb.allows_request(), "Expected circuit breaker to reject requests");
}

#[then("the circuit breaker should allow requests")]
async fn then_cb_allows(world: &mut StepWorld) {
    let cb = world.circuit_breaker.as_mut().expect("No circuit breaker");
    assert!(cb.allows_request(), "Expected circuit breaker to allow requests");
}
...
async fn given_retry_policy(world: &mut StepWorld, max_retries: u32, backoff_base_ms: u64) {
    world.stored_ids.insert("retry_max".to_string(), max_retries.to_string());
    world.stored_ids.insert("retry_backoff".to_string(), backoff_base_ms.to_string());
}
...
async fn when_execute_with_resilience(world: &mut StepWorld) {
    ...
    let max_retries: u32 = world.stored_ids.get("retry_max").unwrap_or(&"3".to_string()).parse().unwrap();
    let backoff_base_ms: u64 = world.stored_ids.get("retry_backoff").unwrap_or(&"10".to_string()).parse().unwrap();
    ...
    match execute_with_resilience(&cmd.step_type, &cmd.config, &cmd.input_data, &retry_policy, &mut cb).await {
        Ok(output) => {
            world.stored_ids.insert("exec_output".to_string(), serde_json::to_string(&output).unwrap());
            world.stored_ids.insert("exec_success".to_string(), "true".to_string());
        }
