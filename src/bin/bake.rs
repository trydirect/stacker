...
//! health-gate it and snapshot it. Prints the BakeRecord (with the image_id user
//! deploys will clone from).
//!
//! Usage:
//!   HETZNER_TOKEN=... cargo run --bin bake -- \
//!     --ip <build-box-ip> --stack lamp --version v1 --health-url http://<ip>/health
//!
//! Without --health-url the health gate is skipped (assumed healthy) — fine for
//! a first smoke test, but real bakes should always pass a health URL.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--ip" => { ip = args.get(i + 1).cloned(); i += 2; }
            "--server-id" => { server_id = args.get(i + 1).and_then(|v| v.parse().ok()); i += 2; }
            "--stack" => { stack = args.get(i + 1).cloned().unwrap_or(stack); i += 2; }
            "--version" => { version = args.get(i + 1).cloned().unwrap_or(version); i += 2; }
            "--health-url" => { health_url = args.get(i + 1).cloned(); i += 2; }
    ...
    let record = run_bake(&connector, &token, target, &stack, &version, healthy, &detail)
        .await
