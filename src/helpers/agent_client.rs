...
impl AgentClient {
    ...
    async fn post_signed_json<T: Serialize>(
        &self,
        path: &str,
        body: &T,
    ) -> Result<Response, reqwest::Error> {
    ...
    pub async fn rotate_token(&self, new_token: &str) -> Result<Response, reqwest::Error> {
        ...
        self.post_signed_json("/api/v1/auth/rotate-token", &body)
            .await
    }
