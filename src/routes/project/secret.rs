...
async fn fetch_owned_app(
    pool: &PgPool,
    user: &models::User,
    project_id: i32,
    app_code: &str,
) -> Result<models::ProjectApp, actix_web::Error> {
    ...
    db::project_app::fetch_by_project_and_code(pool, project_id, app_code)
        .await
        .map_err(JsonResponse::internal_server_error)?
...
pub async fn list(
    ...
    let _app = fetch_owned_app(pg_pool.get_ref(), &user, project_id, &code).await?;
