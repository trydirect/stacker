...
/// Update a step execution located by its correlation key `(pipe_execution_id, step_id)`
/// rather than the surrogate row id. Used by the AMQP result consumer, which only
/// knows the pair carried on `StepResultMsg`. Relies on the unique index
/// `idx_dag_step_exec_lookup` to resolve to a single row.
#[tracing::instrument(name = "Update step execution status by step", skip(pool))]
pub async fn update_step_execution_by_step(
    pool: &PgPool,
    pipe_execution_id: &Uuid,
    step_id: &Uuid,
    status: &str,
    ...
    let span = tracing::info_span!("Updating DAG step execution status by step");
    let now = chrono::Utc::now();
    sqlx::query_as::<_, DagStepExecution>(
        r#"
        UPDATE pipe_dag_step_executions SET
            ...
            started_at = CASE WHEN $3 = 'running' AND started_at IS NULL THEN $6 ELSE started_at END,
            completed_at = CASE WHEN $3 IN ('completed', 'failed', 'skipped') THEN $6 ELSE completed_at END
        WHERE pipe_execution_id = $1 AND step_id = $2
        RETURNING id, pipe_execution_id, step_id, status, input_data, output_data, error, started_at, completed_at, created_at
        "#,
    )
    .bind(pipe_execution_id)
    ...
    .await
    .map_err(|err| {
        tracing::error!("Failed to update step execution by step: {:?}", err);
        format!("Failed to update step execution by step: {}", err)
    })
}
